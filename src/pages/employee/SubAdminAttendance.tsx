...
export default function SubAdminAttendance() {
  ...
  const startCamera = async () => {
    ...
    try {
      // Step 1: Check if any camera device is available on this system
      let hasCamera = false;
      try {
        const devices = await navigator.mediaDevices.enumerateDevices();
        hasCamera = devices.some((d) => d.kind === "videoinput");
      } catch {
        // enumerateDevices not supported — proceed and let getUserMedia decide
        hasCamera = true;
      }

      if (!hasCamera) {
        setCameraError(
          "No camera found on this device. Please connect a webcam and refresh the page."
        );
        return;
      }
