...
async function main() {
  ...
  if (!r2Available) {
    console.warn("⚠️ Warning: Cloudflare R2 is not configured. Release manifests will be generated locally and APK will be published to GitHub Releases.");
  }
  ...
  // Save manifest locally for web/repo distribution
  ...
  // Step 4: If R2 is configured, upload to R2
  if (r2Available) {
  ...
  console.log(`R2 Upload:     ${r2Available ? "Uploaded to " + getBucketName() : "Skipped (R2 not configured)"}`);
