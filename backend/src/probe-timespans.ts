...
async function probe() {
  console.log("=== PROBING PlantDetailAPI.do TIMESPANS ===");
  ...
  const tests = [
    // 1. type=1 (monthly data of a year?) with YYYY
    {
      label: "type=1 (day) with YYYY (2026)",
      url: `${baseUrl}/PlantDetailAPI.do?plantId=${plantId}&type=1&date=2026`,
    },
    // 2. type=1 with YYYY-MM
    {
      label: "type=1 (day) with YYYY-MM (2026-05)",
      url: `${baseUrl}/PlantDetailAPI.do?plantId=${plantId}&type=1&date=2026-05`,
    },
    // 3. type=2 (yearly data of plant?) with YYYY
    {
      label: "type=2 (month) with YYYY (2026)",
      url: `${baseUrl}/PlantDetailAPI.do?plantId=${plantId}&type=2&date=2026`,
    },
    // 4. type=3 (yearly?) with YYYY
    {
      label: "type=3 (year) with YYYY (2026)",
      url: `${baseUrl}/PlantDetailAPI.do?plantId=${plantId}&type=3&date=2026`,
    },
    // 5. type=3 with YYYY-MM-DD
    {
      label: "type=3 with YYYY-MM-DD",
  ...
  for (const t of tests) {
    ...
    try {
      ...
      if (!isHtml) {
        console.log("Success! Data keys:", Object.keys(d.back?.data || {}));
        console.log("Response JSON:", JSON.stringify(d, null, 2).slice(0, 400));
      }
    } catch (e: any) {
