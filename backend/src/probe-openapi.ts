...
async function probe() {
  console.log("=== PROBING OPENAPI.GROWATT.COM (MOBILE API) ===");
  const baseUrl = "https://openapi.growatt.com";
  const username = "R Nagpure";
  const password = "Swapnila@09";

  // Calculate the custom MD5 hash algorithm used by Growatt Mobile App
  ...
  try {
    ...
    console.log("Login Response Body:", JSON.stringify(loginRes.data));
    
    if (loginRes.data?.back?.success) {
      sessionCookie = extractCookies(loginRes);
      userId = String(loginRes.data.back.user.id);
      console.log(`✓ Login Success! userId=${userId}, cookies=${sessionCookie}`);
    } else {
      console.error("Login rejected by Growatt mobile API!");
      return;
    }
  } catch (e: any) {
  ...
  try {
    ...
    const plants = plantListRes.data?.back || [];
    if (plants.length > 0) {
