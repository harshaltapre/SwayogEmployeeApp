...
async function verifyLogin(usr: string, pass: string) {
  ...
  const pwdBase64 = encryptPassword(pass);
  
  console.log(`\n========================================`);
  console.log(`Testing OSS Base64 Login: usr=${usr}, pass=${pass}`);
  console.log(`Payload: account=${usr}&password=${pwdBase64}`);
  console.log(`========================================`);

  try {
    const res = await axios.post(
      `${baseUrl}/login`,
      `account=${encodeURIComponent(usr)}&password=${encodeURIComponent(pwdBase64)}`,
      {
        headers: {
    ...
    if (typeof resTitle.data === "string" && resTitle.data.includes("dumpLogin")) {
      console.log(`-> Verification FAILED (redirected to dumpLogin)`);
    } else {
...
async function main() {
  // Test case A: rjm9984 / e95c3093
  let cookie = await verifyLogin("rjm9984", "e95c3093");
  
  // Test case B: rjm9984 / lm6t7e69dm9630mniq134341
  if (!cookie) {
